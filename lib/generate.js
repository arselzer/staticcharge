...
module.exports = function generate(cwd, cb) {
  ...
  async.series([
  	function(cb) {
  		...
  		fs.readFile(path.join(__dirname, "..", "default", "package.json"), function(err, content) {
  	...
  	function(cb) {
  		rl.question("Install npm dependencies now? (y/n)", function(answer) {
    ...
    function(cb) {
      rl.question("Install bower dependencies now? (y/n)", function(answer) {
    		if (/^\s*(y|yes)\s*$/.test(answer)) {
